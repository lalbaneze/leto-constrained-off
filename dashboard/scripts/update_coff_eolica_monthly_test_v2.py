...
# >>> NOVOS OUTPUTS (V2) <<<
OUT_MONTHLY_TEST_V2 = os.path.join(DATA_DIR, "coff_eolica_monthly_test_v2.csv")
OUT_RAW_TEST_V2 = os.path.join(RAW_DIR, "coff_eolica_raw_citi_test_v2.csv")
...
def build_monthly_v2_from_cached_csvs():
    ...
    for f in files:
        try:
            ...
            # ======= CÁLCULO CITI-LIKE (igual ao seu) =======
            ...
            # geração sempre conta, independente do código
    ...
    # =========================
    # V2 (ROTA 1): AGREGA POR MÊS + USINA (NÃO por código)
    # =========================
    gen = (
        raw.groupby(["mes", "nom_usina"], as_index=False)
           .agg(
               generation_mwh=("generation_mwh", "sum"),
               last_instante=("instante", "max")
           )
    )
    ...
    monthly = gen.merge(curt, on=["mes", "nom_usina"], how="left").fillna(0.0)
    ...
    monthly["last_instante"] = pd.to_datetime(monthly["last_instante"], errors="coerce").dt.strftime("%Y-%m-%d %H:%M:%S")

    os.makedirs(RAW_DIR, exist_ok=True)
    raw.to_csv(OUT_RAW_TEST_V2, index=False, encoding="utf-8")
    monthly.to_csv(OUT_MONTHLY_TEST_V2, index=False, encoding="utf-8")

    print("\n✅ OK (V2)")
    print("Gerados:")
    print(" -", OUT_MONTHLY_TEST_V2)
    print(" -", OUT_RAW_TEST_V2)
    print("Falhas:", fail)
    print("Linhas raw:", len(raw))
    print("Linhas monthly v2:", len(monthly))
...
def main():
    ...
    print("Construindo monthly TESTE V2 (usina-level)...")
    build_monthly_v2_from_cached_csvs()
